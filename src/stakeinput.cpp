...
// The zKYDC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zKYDC is a hash of the serial
...
{
    //Create an output returning the zKYDC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZKYDOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zKYDC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zKYDC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZKYDOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zKYDC output", __func__);
...
bool CZPivStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    ...
    CMintMeta meta;
    if (!zkydTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zkydTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a KYDC stake is the outpoint
