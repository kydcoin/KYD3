...
public:
    ...
    bool WriteZKYDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZKYDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZKYDSeed_deprecated(uint256& seed);
    bool EraseZKYDSeed();
    bool EraseZKYDSeed_deprecated();

    bool WriteZKYDCount(const uint32_t& nCount);
    bool ReadZKYDCount(uint32_t& nCount);
