...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 KYDC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzKYDReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzKYDReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzKYDReset.connect(boost::bind(NotifyzKYDReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzKYDReset.disconnect(boost::bind(NotifyzKYDReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zkydTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
