                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zKYD ought to be enough for anybody." - Bill Gates, 2017
    ui->zKYDpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzKYDSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zKYDpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzKYD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zKYD, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonSpendzKYD_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zKYD, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zKYD
        sendzKYD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zKYD
    sendzKYD();
}

void PrivacyDialog::on_pushButtonZKydControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZKydControlDialog* zKydControl = new ZKydControlDialog(this);
    zKydControl->setModel(walletModel);
    zKydControl->exec();
}

void PrivacyDialog::setZKydControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzKydSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzKYD()
{
    ...
    double dAmount = ui->zKYDpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zKYD is requested
    ...
    // use mints from zKYD selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZKydControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZKydControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zKYD was spent successfully update the addressbook with the label
    ...
    // Clear zkyd selector in case it was used
    ZKydControlDialog::setSelectedMints.clear();
    ui->labelzKydSelected_int->setText(QString("0"));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zkydTracker->GetMints(false);
...
{
    ui->labelzKYDSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzKYD->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonSpendzKYD->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
