...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeKYDAmount"))
        settings.setValue("nAnonymizeKYDAmount", 1000);

    nAnonymizeKYDAmount = settings.value("nAnonymizeKYDAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKYDAmount"))
        SoftSetArg("-anonymizekydamount", settings.value("nAnonymizeKYDAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kyd.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKYDAmount:
            return QVariant(nAnonymizeKYDAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKYDAmount:
            nAnonymizeKYDAmount = value.toInt();
            settings.setValue("nAnonymizeKYDAmount", nAnonymizeKYDAmount);
            emit anonymizeKYDAmountChanged(nAnonymizeKYDAmount);
            break;
        case CoinControlFeatures:
