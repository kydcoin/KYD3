...
CzKYDWallet::CzKYDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkyd seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKYDSeed_deprecated(seed)) {
...
bool CzKYDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKYDCount(nCountLastUsed);
    else if (!walletdb.ReadZKYDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKYDWallet::Lock()
{
    seedMaster = 0;
}

void CzKYDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKYDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKYD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKYDWallet::LoadMintPoolFromDB()
{
...
void CzKYDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKYDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKYDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkydTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKYDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKYD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkydTracker which also adds to database
    pwalletMain->zkydTracker->Add(dMint, true);
...
void CzKYDWallet::SeedToZKYD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKYDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKYDWallet::GenerateDeterministicZKYD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKYDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKYDWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzKYDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
