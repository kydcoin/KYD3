...
COIN_TGZ='curl -s https://api.github.com/repos/enginama/KYD3/releases/latest | grep "browser_download_url.*\-Ubuntu16\-x86\_64\.tar\.gz"| cut -d '"' -f 4'
echo $COIN_TGZ
fi
if [[ $(lsb_release -d) = *18.04* ]]; then
COIN_TGZ='curl -s https://api.github.com/repos/enginama/KYD3/releases/latest | grep "browser_download_url.*\-Ubuntu18\-x86\_64\.tar\.gz"| cut -d '"' -f 4'
fi
